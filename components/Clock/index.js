...
export default function Clock() {
  ...
  const drawClock = (ctx) => {
    ...
    drawLine(ctx, center, center, x2(time.hours, 12, center, lineRad), y2(time.hours, 12, center, lineRad), lineWidth); // hour
    drawLine(ctx, center, center, x2(time.minutes, 60, center, lineRad), y2(time.minutes, 60, center, lineRad), lineWidth); // minutes
  ...
  useEffect(()=>{
    ...
    let timeOut = setInterval(() => {drawClock(ctx)}, 1000);
  ...
  return (
    <canvas hidden ref={canvasRef} width={canvasSize} height={canvasSize}></canvas>
  )
}
